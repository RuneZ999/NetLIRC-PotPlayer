...
This is a AHK https://www.autohotkey.com script that offers a remote control on your mobile devices, or anything that has a web browser. It uses HTML5, CSS3, JavaScript and AJAX along the AHK, still you only need to have AHK installed only.
...
AHKhttp , I use a modified version that fixes a memory leak called AHKhttp_MemoryLeakFix
...
Font Awesome fontello selection
...
• Install AutoHotKey https://www.autohotkey.com • Run the script in one of the two modes

interactive mode:￼

command line mode: Specify the port number as a command line argument
...
• Edit the Network_Pot_Player.ahk
...
Now lets assume we want to add a event called MyCommand we add this line after:
...
200 is the standard response for successful HTTP requests. 404 is page or resource not found, it is handled already in the script.
1. Open HTML.html
...
|html|what it does| | - |--| |btn-primary|is button collor from bootstrap| |id="id_mycommand"|id_mycommand - is used to trigger the highlight| |httpGet("/mycommand")|/mycommand - is the http get request that triggers the script| |#000|black color in hex format for confirmed execution| |#f00|red color in hex format for confirmed execution| | <i class="icon-my-command"></i> |icon-my-command is a icon from font awesome, embedded with http://fontello.com , exclude the i tag for no icon| | My Command |the name of the button starts with a space between icon and text|
